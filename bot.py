...
class BlumTod:
    ...
    def renew_access_token(self, tg_data):
        ...
        self.log(f"{hijau}Erişim Yetkisi Başarılı ")
        return access_token
    ...
    def solve_task(self, access_token):
        ...
        for task in res.json():
            ...
            self.log(f"{kuning}Görev Zaten Yapılmış : {task_title} !")
    ...
    def get_balance(self, access_token, only_show_balance=False):
        ...
        res = self.http(url, headers)
        balance = res.json()["availableBalance"]
        self.log(f"{hijau}Bakiye : {putih}{balance}")
        ...
        self.log(
            f"{kuning}Şuanda Farm Yapılamıyor. Lütfen Sürenin Bitmesini Bekleyin !"
        )
        end_date = datetime.fromtimestamp(end_farming)
        self.log(f"{hijau}Son Farm Zamanı : {putih}{end_date}")
    ...
    def get_friend(self, access_token):
        ...
        self.log(f"{putih}Davet Limiti : {hijau}{limit_invite}")
        self.log(f"{putih}Davet Başına Kazanç : {hijau}{amount_claim}")
        if can_claim:
    ...
    def playgame(self, access_token):
        ...
        self.log(f"{hijau}Sahip olduğun {putih}{play}{hijau} adet oyun hakkı var !")
        for i in range(play):
    ...
    def countdown(self, t):
        while t:
            ...
            print(
                f"{putih}Sürenin Bitmesi Bekleniyor {jam}:{menit}:{detik} ",
                flush=True,
                end="\r",
            )
            t -= 1
    ...
    {hijau}NOT : {putih}Lütfen Bot Güncellemelerini Takip Edin !
        ...
        self.log(f"{hijau}Toplam Hesap : {putih}{len(datas)}")
        if len(datas) <= 0:
        ...
        while True:
            ...
            for no, data in enumerate(datas):
                self.log(f"{hijau}Hesap Numarası - {putih}{no + 1}")
                data_parse = self.data_parsing(data)
                user = json.loads(data_parse["user"])
                self.log(f"{hijau}Giriş Yapan Hesap : {putih}{user['first_name']}")
